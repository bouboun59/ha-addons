...
# Generate config file for localv2 connection plugin
echo "
_MQTT_ip = '$(bashio::config 'MQTT_ip')'
_MQTT_port = $(bashio::config 'MQTT_port')
_MQTT_authentication = $(bashio::config 'MQTT_authentication')
...
elif bashio::config.true 'USE_MCZ_LOCALV2'; then
    bashio::log.info "Launching maestro localv2 connection plugin"
    python3 /maestro/localv2/maestro.py
else
